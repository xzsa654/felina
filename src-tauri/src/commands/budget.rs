...
pub fn set_budget(
    ...
    fs::write(glyphic_settings_path(), content).map_err(|e| format!("failed to write: {e}"))
}
...
pub fn get_cost_summary() -> Result<CostSummary, String> {
    ...
    fn day_cost(tokens: &HashMap<String, f64>) -> f64 {
        ...
        tokens
            ...
            .map(|(model, count)| {
                ...
                let per_m = price_output(model) * 0.3
                    + price_input(model) * 0.1
                    + price_cache_read(model) * 0.6;
                (count / 1_000_000.0) * per_m
            })
            .sum()
    }

    let today_cost = daily_model_tokens
        .iter()
        .find(|(d, _)| d == &today)
        .map(|(_, t)| day_cost(t))
        .unwrap_or(0.0);

    let month_cost: f64 = daily_model_tokens
        .iter()
    ...
    // Last 7 days
    let last_7: Vec<f64> = daily_model_tokens
        .iter()
        .rev()
        .take(7)
        .map(|(_, t)| day_cost(t))
    ...
    let monthly_projection = if day_of_month > 0.0 {
        month_cost / day_of_month * 30.0
    } else {
        0.0
    };
    ...
    let mut per_project_month: Vec<ProjectCost> = project_costs
        .into_iter()
        .map(|(project, (cost, messages))| ProjectCost {
            project,
            cost,
            messages,
        })
        .collect();
    per_project_month.sort_by(|a, b| {
        b.cost
            .partial_cmp(&a.cost)
