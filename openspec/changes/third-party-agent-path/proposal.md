...
- 新增 `agent_path_remove` command：接受 agent key，掃描所有 skill 的 sync-meta 移除對應 targets，不刪磁碟檔案
...
- 刪除 custom agent 前顯示影響清單（N 個 skill 的 M 個 targets 會被移除），確認後呼叫 `agent_path_remove` command
...
- 刪除 agent path 時自動清除磁碟上已 push 的檔案——只清 sync-meta targets

## Capabilities

### New Capabilities

- `custom-agent-path`: 使用者可在 Settings 新增、編輯、刪除第三方 agent 路徑，fan-out 使用 generic renderer，刪除時清理對應 targets
