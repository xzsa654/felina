...
> **Snapshot disclaimer**: This spec is a time-bound research snapshot. Its contents reflect each covered agent vendor's skill format as documented on the source URLs and `verified YYYY-MM-DD` dates cited inline below. When an agent vendor publishes a major release that changes their skill format (new frontmatter field, renamed discovery path, load-mechanism rewrite, etc.), every affected fact in this spec MUST be re-verified and the `verified YYYY-MM-DD` date refreshed. Stale facts are worse than no facts.
>
> **Covered agent vendors** (initial inventory):
>
> - Anthropic Claude (Claude Code CLI)
> - OpenAI Codex CLI
> - Google Gemini (`gemini-cli` → Antigravity CLI; see Product lineage note inside the Requirement)
>
> Adding a fourth vendor: see the **Extension Template for New Agents** Requirement below.
