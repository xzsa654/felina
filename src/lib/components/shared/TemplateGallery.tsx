...
const TEMPLATES: Template[] = [
  ...
  { id: "rule-a11y", name: "Accessibility", description: "UI accessibility standards", category: "rule", paths: ["**/*.tsx", "**/*.jsx"], content: "# Accessibility Rules\n\n- All images must have alt text\n- Use semantic HTML elements\n- Ensure keyboard navigation works\n- Maintain sufficient color contrast\n- Add aria-labels to icon-only buttons" },
...
export default function TemplateGallery({ open, defaultCategory = "all", onselect, onclose }: Props) {
  const [activeCategory, setActiveCategory] = useState<Template["category"] | "all">(defaultCategory);
  ...
  return (
    <>
      <button className="fixed inset-0 bg-black/50 z-50" onClick={onclose} aria-label="Close gallery" />
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-bg-secondary border-t border-border rounded-t-2xl shadow-2xl" style={{ height: "65vh" }}>
        ...
        <div className="flex items-center justify-between px-6 pb-3">
          <div>
            ...
            <p className="text-xs text-text-muted">{TEMPLATES.length} templates across {CATEGORIES.length - 1} categories</p>
          </div>
          <button className="p-2 text-text-muted hover:text-text-primary rounded-lg hover:bg-bg-hover" onClick={onclose} aria-label="Close">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-6 pb-3 space-y-2">
          <div className="flex gap-1 overflow-x-auto">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  key={cat.id}
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg shrink-0 transition-colors ${activeCategory === cat.id ? "bg-accent text-white" : "bg-bg-tertiary text-text-muted hover:text-text-secondary"}`}
          ...
          <div className="relative">
            ...
            <input
              type="text"
              className="w-full pl-9 pr-3 py-2 text-sm bg-bg-tertiary border border-border rounded-lg text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent"
        ...
        <div className="px-6 pb-6 overflow-y-auto" style={{ height: "calc(65vh - 170px)" }}>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            {filtered.map((template) => (
              <button
                key={template.id}
                className="text-left p-4 bg-bg-tertiary border border-border rounded-lg hover:border-accent/30 transition-colors group"
                onClick={() => { onselect(template); onclose(); }}
              >
                <div className="flex items-start justify-between mb-2">
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${CATEGORY_COLORS[template.category]}`}>{template.category}</span>
                  <span className="text-[10px] text-accent opacity-0 group-hover:opacity-100 transition-opacity">+ Add</span>
                </div>
          ...
          {filtered.length === 0 && (
            <p className="text-sm text-text-muted text-center py-8">No templates match your search</p>
          )}
        </div>
      </div>
    </>
